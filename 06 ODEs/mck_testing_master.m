...
for i=1:length(cvec)
    ...
    [~,xc(i,:),vc(i,:)] = mck(m,cvec(i),k,u,t,x0,v0,options);
end
...
type = 'Pulsed';     % Change to either 'Constant', 'Harmonic', or 'Pulsed'
...
switch type
    ...
    case 'Harmonic'
        % Harmonic forcing
        w = 1;          % Frequency of sinusoidal forcing [rad/s]
        u = @(t) u0*sin(w*t);

    case 'Pulsed'
        % Pulsed forcing
        t_on = 20;
        u = @(t) u0*(heaviside(t) - heaviside(t-t_on));
end
