...
% Numerical solution
[~,P] = population(t,P0,a,b,options);       % Population history [people]         

% Find year in which we hit or surpass P99
P_ss = a/b;       % Steady-state population [people]
P99 = 0.99*P_ss;
t99 = min(t(P >= P99));                 
...
aData = [25:5:35]*1e-3;
b = 2.9e-12;
Pa = zeros(length(aData),length(t));   % Preallocate matrix to store the population calculations for all "a" cases

% Write for loop to iterate through each case of a
for i = 1:length(aData)
    [t,Pa(i,:)] = population(t,P0,aData(i),b,options);  % Call population function for each case of aData
end
...
bData = [2.5 3 3.5]*1e-12;
a = 0.029;  
Pb = zeros(length(bData),length(t));   % Preallocate matrix to store the population calculations for all "b" cases

% Write for loop to iterate through each case of b
for i = 1:length(bData)
    [t,Pb(i,:)] = population(t,P0,a,bData(i),options);  % Call population function for each case of aData
end
