...
u = @(t) ((-F1/t1)*t + F1) + ((F1/t1)*t - F1).*heaviside(t-t1);
...
%% Single c Testing
c_max = 2*sqrt(m*k);                        % Max c value (Requirement 2) [N*s/m]
c_test = 4                                 % Test a c value

% Call mck function, then determine if requirements are met
[~,x,v] = mck(m,c_test,k,u,t,IC(1),IC(2),options);  % Call mck function
[R1_met,v_maxaftert1,R2_met] = checkRequirements(t,v,t1,v_min,v_max,c_test,c_max);   % Check Req's
...
for i=1:length(c)
    [t,x,v] = mck(m,c(i),k,u,t,IC(1),IC(2),options);
    [R1_met(i),v_maxaftert1(i),R2_met(i)] = checkRequirements(t,v,t1,v_min,v_max,c(i),c_max);
end
toc
...
min_idx = find(R1_met == 1,1);          % Obtain index of the minimum allowable c
c_min = c(min_idx)     

max_idx = find(R1_met == 1,1,'last');   % Obtain index of the maximum allowable c
c_max = c(max_idx)
...
% Check Requirement 1
v_maxaftert1 = max(v(t>t1));
R1_met = ((v_maxaftert1 >= v_min) && (v_maxaftert1 <= v_max));
